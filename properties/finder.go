...
func FindByKey(key string) (*KeyProperties, error) {
	...
	rows, err := db.Query(`
		SELECT 
			p.property_id, p.key, p.name, p.type, pv.property_values_id,
			pv.value, pv.count
	...
	for rows.Next() {
		err = rows.Scan(&property_id, &_key, &name, &_type, &property_values_id,
			&value, &count)
		...
		if property := keyProperties.GetProperty(name); property != nil {
			...
			property = keyProperties.AddProperty(property_id, _key, name, _type)
			property.AddValue(property_values_id, value, count)
		}
	}
